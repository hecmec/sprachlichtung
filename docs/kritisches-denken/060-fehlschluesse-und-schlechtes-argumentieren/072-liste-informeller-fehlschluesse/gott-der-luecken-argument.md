---
title: "Gott-der-Lücken-Argument"
title_meta: "Gott-der-Lücken-Argument"
sidebar_label: Gott-der-Lücken-Argument
sidebar_position: 1
description: "Beschreibung des Gott-der-Lücken-Arguments (God of the Gaps)"
keywords:
  - Fehlschlüsse
  - Fallacies
  - kritisches Denken
  - Gott-der-Lücken
  - God of the Gaps
last_update:
  date: 2026-06-08
  author: Immanuel Lupinus
completeness: 4/5
draft: false
# slug:
# image:
---

:::tip Kurz
Was die Wissenschaft (noch) nicht erklärt, wird kurzerhand einer übernatürlichen Ursache zugeschrieben.

_Wo das Wissen endet, beginnt die Behauptung._
:::

## Definition

Das **Gott-der-Lücken-Argument** (_god of the gaps_) füllt eine **Erklärungslücke** des aktuellen Wissens mit einer übernatürlichen Ursache: Weil etwas (noch) nicht erklärt ist, wird ein Eingreifen Gottes (oder einer anderen letzten Instanz) als Erklärung angenommen.

Der Fehler: Eine Wissenslücke ist kein Beleg für eine bestimmte Erklärung. „Wir wissen es noch nicht“ ist nicht dasselbe wie „es muss übernatürlich sein“. Solche Lücken sind historisch immer wieder durch natürliche Erklärungen geschlossen worden.

Der Fehlschluss hat die folgende Form:

1. Das Phänomen P ist (derzeit) nicht erklärt.
2. Mir fällt keine natürliche Erklärung ein.
3. Also ist die Ursache von P übernatürlich.

EN: _god of the gaps_ / _divine fallacy_

## Verwandtschaft

<!-- zähle ähnliche Fehlschlüsse auf -->

- **Persönliche Skepsis** &mdash; die Wurzel: aus „ich kann es mir nicht erklären“ wird ein Schluss.
- **Argumentum ad Ignorantiam** &mdash; Oberbegriff: aus Nichtwissen wird auf eine bestimmte These geschlossen.
- **Falsches Dilemma** &mdash; verwandt: so getan, als gäbe es nur „erklärt“ oder „übernatürlich“.

## Beispiele

### Beispiel 1

<!-- Beschreibung Beispiel eins -->

> „Die Wissenschaft kann nicht erklären, wie das Leben genau entstand &mdash; also muss Gott es geschaffen haben.“

Eine offene Forschungsfrage ist kein Beleg für eine bestimmte Antwort. Die Lücke wird vorschnell gefüllt.

### Beispiel 2

<!-- Beschreibung Beispiel zwei -->

> „Niemand weiss, warum die Naturkonstanten genau diese Werte haben &mdash; das kann nur Absicht sein.“

Aus dem fehlenden Wissen über die Ursache folgt keine planende Instanz.

## Quellen

<!-- Wo wird dieser Fehlschluss beschrieben. (Wikipedia, etc) -->

- [Wikipedia: God of the gaps](https://en.wikipedia.org/wiki/God_of_the_gaps)
- [Logically Fallacious: God of the Gaps](https://www.logicallyfallacious.com/logicalfallacies/God-of-the-Gaps)

[Article en français](/fr/docs/kritisches-denken/060-fehlschluesse-und-schlechtes-argumentieren/072-liste-informeller-fehlschluesse/gott-der-luecken-argument.md) | [Article in English](/en/docs/kritisches-denken/060-fehlschluesse-und-schlechtes-argumentieren/072-liste-informeller-fehlschluesse/gott-der-luecken-argument.md)
