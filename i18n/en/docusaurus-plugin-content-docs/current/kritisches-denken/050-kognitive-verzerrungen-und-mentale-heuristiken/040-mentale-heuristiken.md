---
title: Mental Heuristics
sidebar_position: 4
description: Explanation of mental heuristics as cognitive shortcuts, including their advantages and disadvantages
keywords:
  - mental heuristics
  - representativeness heuristic
  - simulation heuristic
  - affect heuristic
  - cognitive shortcuts
  - decision-making
last_update:
  date: 2026-01-28
  author: Immanuel Lupinus
translation_status: mixed
---

<!--t src=f4266b64-->
<!-- markdownlint-disable MD024 -->

<!--t src=6f65c40a-->
Mental heuristics are cognitive shortcuts or rules of thumb that we use to make judgments or decisions quickly. They are not bad per se &mdash; in fact they are often useful and efficient. However, under certain circumstances they can lead to systematic errors.

<!--t src=8f19dbef-->
## 1. Representativeness Heuristic

<!--t src=38452daf-->
The **representativeness heuristic** is the tendency to judge the probability of an event based on how much it resembles a typical example or prototype.

<!--t src=da615aa6-->
### Example

<!--t src=3f349433-->
Linda is 31 years old, single, eloquent, and very intelligent. She studied philosophy. As a student, she was deeply engaged in issues of discrimination and social justice.

<!--t src=5731da56-->
Which is more likely?
a) Linda is a bank teller.
b) Linda is a bank teller and active in the feminist movement.

<!--t src=76c7d906-->
Many people choose option b), even though this is mathematically impossible (the probability of a conjunction can never be higher than the probability of one of its parts). This happens because Linda's description fits the stereotype of a feminist better than that of a bank teller.

<!--t src=a3b976cf-->
### Problems

<!--t src=f9c84bb7-->
- Neglect of base rates (how common something is in the general population)
- Overemphasis on similarity to stereotypes
- Conjunction fallacy (as in the Linda example)

<!--t src=0b65c7da-->
## 2. Simulation Heuristic

<!--t src=d8564ff4-->
The **simulation heuristic** refers to our tendency to judge the probability of an event based on how easily we can imagine it occurring.

<!--t src=da615aa6-->
### Example

<!--t src=e6115491-->
People consider it more likely to die in a plane crash than from a disease that shows no symptoms because they can imagine a plane crash more easily and vividly.

<!--t src=a3b976cf-->
### Problems

<!--t src=eca54ec1-->
- Overestimation of the probability of easily imaginable events
- Underestimation of the probability of abstract or complex events
- Distorted risk perception

<!--t src=6de3e449-->
## 3. Affect Heuristic

<!--t src=2a1e44de-->
The **affect heuristic** describes how emotional reactions (affects) influence judgments and decisions.

<!--t src=da615aa6-->
### Example

<!--t src=79ef49cf-->
People judge risks as higher when they associate negative feelings with them and as lower when they associate positive feelings with them. For instance, technologies that evoke fear (e.g. nuclear power) are often perceived as riskier than those that evoke positive associations, even when objective data paint a different picture.

<!--t src=a3b976cf-->
### Problems

<!--t src=7a6e1cfe-->
- Emotional reactions can override rational assessments
- Manipulation through emotional appeals
- Inconsistent risk assessments

<!--t src=0991efb6-->
## Advantages and Disadvantages of Heuristics

<!--t src=b586c685-->
### Advantages

<!--t src=97110fb1-->
- Speed: Heuristics enable quick decisions without extensive analysis.
- Efficiency: They save cognitive resources for more complex tasks.
- Practicality: In many everyday situations, they provide sufficiently good results.
- Adaptability: They evolved to be useful in specific environments.

<!--t src=a09a520b-->
### Disadvantages

<!--t src=5f6117b7-->
- Systematic errors: Under certain conditions, they lead to predictable mistakes.
- Inappropriateness: In complex or unfamiliar situations, they can lead to poor decisions.
- Unconsciousness: We are often unaware that we are using heuristics.
- Resistance: They are difficult to overcome, even when we are aware of them.
