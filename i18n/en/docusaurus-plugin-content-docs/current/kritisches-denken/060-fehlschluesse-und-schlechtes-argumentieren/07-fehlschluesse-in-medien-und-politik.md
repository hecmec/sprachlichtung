---
title: Fallacies in Media and Politics
sidebar_position: 7
description: Examination of typical fallacies in the media and political discourse, and strategies for recognising them
keywords:
  - media fallacies
  - political fallacies
  - false balance
  - anecdotal evidence
  - emotional appeals
  - ad hominem in politics
  - false dichotomies
  - media criticism
last_update:
  date: 2026-01-29
  author: Immanuel Lupinus
translation_status: auto
custom_translation_overwritten: false
---

<!--t src=4d624707-->
<!-- ## Fallacies in Media and Politics -->

<!--t src=53b71e83-->
Fallacies are widespread in public discourse, especially in the media and politics. Here are some examples and strategies for recognising them:

<!--t src=0c0c3a0e-->
## Common Fallacies in the Media

<!--t src=d48d6507-->
1. **False balance**: Presenting scientific consensus and marginal opposing opinions as equivalent positions.

<!--t src=6ec6d83f-->
   **Example:** "Some scientists say that climate change is man-made, others dispute this." (In fact there is an overwhelming scientific consensus on man-made climate change.)

<!--t src=f55e0c5a-->
2. **Anecdotal evidence**: Using individual stories or experiences instead of systematic data.

<!--t src=5d60c1d5-->
   **Example:** "My grandfather smoked all his life and lived to be 95, so smoking can't be that harmful."

<!--t src=0c1f28fb-->
3. **Emotional appeals**: Using emotional language or images to replace rational arguments.

<!--t src=082f2a0a-->
   **Example:** Shocking images of individual cases used to justify political measures, without addressing statistical data or context.

<!--t src=cd7ca89f-->
## Common Fallacies in Politics

<!--t src=788a377f-->
1. **Ad hominem and straw man arguments**: Personal attacks and distortion of opponents' positions are particularly common in political debates.

<!--t src=cbb6d7c7-->
   **Example:** "My opponent is a socialist who wants to restrict our freedoms." (Instead of addressing concrete policy proposals.)

<!--t src=2d445006-->
2. **False dichotomies**: Presenting complex problems as simple either-or decisions.

<!--t src=03f6ee60-->
   **Example:** "Either we cut taxes, or the economy will collapse."

<!--t src=08669948-->
3. **Appeal to tradition or novelty**: Justifying or rejecting policies solely on the basis of their age.

<!--t src=cac232fa-->
   **Example:** "We have always done it this way, so we should continue to do it this way." OR "This is a completely new approach, so it must be better."

<!--t src=12b6a9b3-->
## Strategies for Recognising Fallacies in Public Discourse

<!--t src=04fb84fa-->
1. **Ask for evidence**: Are claims supported by relevant and reliable data?

<!--t src=a10dffc6-->
2. **Identify emotional triggers**: Is there an attempt to persuade through emotional language or images rather than through arguments?

<!--t src=7e2fb8c7-->
3. **Look for omitted alternatives**: Are complex situations being inappropriately oversimplified?

<!--t src=41024a93-->
4. **Check the sources**: Are the cited authorities actually experts in the relevant field?

<!--t src=ecec70cd-->
5. **Pay attention to context**: Are quotes or statistics being taken out of context?

<!--t src=697267e7-->
6. **Be sceptical of perfect solutions**: Are complex problems being presented as if they had simple, clear-cut solutions?
