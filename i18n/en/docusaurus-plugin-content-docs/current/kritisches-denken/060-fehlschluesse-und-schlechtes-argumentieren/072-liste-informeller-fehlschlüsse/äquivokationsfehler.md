---
title: Equivocation
title_meta: Equivocation
sidebar_label: Equivocation
sidebar_position: 1
description: Description of the equivocation fallacy (one word in two meanings)
keywords:
  - fallacies
  - critical thinking
  - equivocation
last_update:
  date: 2026-06-08
  author: Immanuel Lupinus
completeness: 4/5
draft: false
translation_status: auto
custom_translation_overwritten: false
---

<!--t src=7b3f0df8-->

:::tip In brief
An ambiguous word is used within one argument in two different meanings.

_The same word, two meanings._
:::

<!--t src=23594b0b-->

## Definition

<!--t src=4b2f4912-->

In **equivocation**, an **ambiguous word** is used within the same argument in **different meanings**. Because the word stays outwardly the same, the inference seems to „fit“ &mdash; in truth the sentences speak of different things.

<!--t src=7d1a08b3-->

The error: the cogency rests only on the same wording, not on a shared meaning. In syllogisms this produces the **Quaternio Terminorum** (a fourth term).

<!--t src=3c24b250-->

The fallacy has the following form:

<!--t src=8ceb1439-->

1. Statement 1 uses the word W in meaning 1.
2. Statement 2 uses the same word W in meaning 2.
3. The inference treats both meanings as one.

<!--t src=cc443331-->

Also known as: _equivocation_

<!--t src=2851dcb3-->

## Related fallacies

<!--t src=4d08b756-->
<!-- list similar fallacies -->

<!--t src=5159cbf9-->

- **Quaternio Terminorum** &mdash; the formal upshot in the syllogism (four instead of three terms).
- **Ambiguity fallacy (amphiboly)** &mdash; related: ambiguity through _sentence structure_ instead of through a word.
- **Distinction without a difference** &mdash; a related game with meanings.

<!--t src=9dbba01b-->

## Examples

<!--t src=a67a7a61-->

### Example 1

<!--t src=5422e642-->
<!-- description of example one -->

<!--t src=f6a39c2f-->

> 1. A law needs a lawgiver.
> 2. The laws of nature are laws.
> 3. Therefore the laws of nature need a lawgiver.

<!--t src=63d8e19c-->

„Law“ means in 1 a _legal regulation_, in 2 a _scientific regularity_. The two meanings are inadmissibly equated.

<!--t src=f350657d-->

### Example 2

<!--t src=c7d6981c-->
<!-- description of example two -->

<!--t src=4f5cb947-->

> „At the end of the tunnel there is light. Light is a form of energy. So at the end of the tunnel there is energy.“

<!--t src=e5ded5a7-->

„Light“ stands first figuratively (for hope, rescue), then physically. The word only seems to connect the sentences.

<!--t src=69b1092f-->

## Sources

<!--t src=63102e9b-->
<!-- Where is this fallacy described. (Wikipedia, etc.) -->

<!--t src=c779f3a4-->

- [Logically Fallacious: Equivocation](https://www.logicallyfallacious.com/logicalfallacies/Equivocation)
